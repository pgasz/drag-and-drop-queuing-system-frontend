...
queues.forEach((queue) => {
    queue.addEventListener('dragover', (e) => {
        ...
        let elementAfterDraggingElement = getElementAfterDraggingElement(
            queue,
            e.clientY
        )
        if (elementAfterDraggingElement) {
            ...
            // wykonuje się wiele razy więc może wypada to zmienić do eventu upuszczania
...
function getElementAfterDraggingElement(queue, yOfDraggingElement) {
    const queueElements = [...queue.querySelectorAll('.element:not(.dragging)')]
